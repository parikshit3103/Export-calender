...
//         return camelCaseHeaders.map((header) => {
//           if (header === "Duration") {
//             return duration; // Include calculated duration
//           }
//           const foundHeader = visibleHeaders.find((h) => toCamelCase(h) === header);
//           return foundHeader ? event[foundHeader] || '-' : '-';
//         });
//       });
...
//         if (header === "Duration") {
//           return totalHours; // Add total hours to the "Duration" column
//         }
//         return header === camelCaseHeaders[0] ? "Total Hours" : "-"; // Add label to the first column
//       });

//       tableData.push(totalRow); // Append the total row to the table data
...
//       allHeaders.add("Duration"); // Add "Duration" to headers
...
//                           key={header}
//                           scope="col"
//                           className="px-3 py-2 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider sm:px-4 sm:py-3 whitespace-nowrap"
//                         >
//                           {header}
//                         </th>
//                       ))}
...
//                               {header === "Duration" ? rowTotalHours : event[header] || <span className="text-gray-400">-</span>}
//                             </td>
//                           ))}
...
export default function ICSViewerPage() {
  ...
  const handleFetchGoogleCalendar = async () => {
    ...
    const parsedEvents = data.map(flattenEventProperties);
    const allHeaders = new Set<string>();
    parsedEvents.forEach((event: CalendarEvent) => Object.keys(event).forEach((key) => allHeaders.add(key)));
    allHeaders.add("Duration");
    const sortedHeaders = Array.from(allHeaders).sort((a, b) => a.localeCompare(b));
    setHeaders(sortedHeaders);
    setVisibleHeaders(sortedHeaders);
    setEvents(parsedEvents);
    setCurrentPage(1);
  };
      ...
      <div className="w-full max-w-[90%]">
        ...
        <div className="flex flex-col gap-3 mb-6 sm:flex-row sm:flex-wrap sm:gap-4 sm:mb-8 w-full">
          <div className="flex items-center gap-2">
            <DatePicker selected={startDate} onChange={setStartDate} placeholderText="Start Date" className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
            <DatePicker selected={endDate} onChange={setEndDate} placeholderText="End Date" className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
            <button
